...
import { cleanHexCode, executeWithTimeout, type interfaces, supabase } from '@scope/shared';
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const guildUsers = users.filter((user) => steamidsInGuild.has(user.steamid64));
  ...
  const entriesPerColumn = interaction.options.getInteger('entries_per_column') ||
    undefined;
  ...
  await interaction.editReply({
    content: `Here's the server leaderboard as of ${readableTime}, delivered to you in ${
