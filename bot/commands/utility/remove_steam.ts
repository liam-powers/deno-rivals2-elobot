...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    await interaction.reply({
      content: 'Successfully deleted your database entry! You will no longer be tracked.',
      ephemeral: true,
    });
  } catch (error) {
    console.error('remove_steam execute error:', error);
    await interaction.reply({
      content: 'Something went wrong when removing you from the database!! Try again.',
      ephemeral: true,
    });
  }
}
