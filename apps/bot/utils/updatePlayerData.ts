...
export async function updatePlayerData(client: Client<boolean>) {
  // 1.
  ...
  const prevUserStats: UserStats[] | undefined = await getLatestUsersStats();
  ...
  await addUserStatsEntries(newUserStats);
  ...
  const steamid64ToNewUserStats: Record<string, UserStats> = {};
